...
async function experimentInit() {
  ...
  // Конвертируем в csv
  let data = '';
  if (dataObj && dataObj.length > 0 && dataObj[0] !== null && dataObj[0] !== undefined) {
    data = [Object.keys(dataObj[0])].concat(dataObj).map(it => {
        return Object.values(it).join(',')
    }).join('\n');
} else {
    data = 'participant,trial,response\n'; // Заголовки по умолчанию
    data += 'no_data,0,no_response'; // Строка-заглушка
}
...
function trial1RoutineEachFrame() {
  return async function () {
    ...
    for (var stim, _pj_c = 0, _pj_a = psychoJS.experiment.background_stims, _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        stim = _pj_a[_pj_c];
        stim.draw();
    }
...
function trial1RoutineEnd(snapshot) {
  return async function () {
    ...
    if ((k === 1)) {
        ...
        delete psychoJS.experiment.background_stims;
    }
    answr_corr = 0;
    psychoJS.experiment.addData("current_target", current_target);
    if (_pj.in_es6("\u043e", current_target)) {
    ...
    psychoJS.experiment.addData("answr_corr", answr_corr);
